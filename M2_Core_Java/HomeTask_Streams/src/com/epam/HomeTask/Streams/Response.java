...
class ProductImplementation implements ProductInterface{
	...
	public List<Product> getProductByCategory(List<Product> product,String category) {
		...
		return product.stream().filter(p->p.getCategory().equalsIgnoreCase("Electronics")).collect(Collectors.toList());
	}
	
	@Override
	public List<Product> getProductByPriceAndCategory(List<Product> product,String category) {
		
		return product.stream().filter((p->p.getPrice()>1000 || p.getCategory().equalsIgnoreCase("Electronics"))).collect(Collectors.toList());
	}
